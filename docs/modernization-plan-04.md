...
9. **提交后元数据幂等**：`PostCommitFinalizer` 在重试循环外创建稳定 ArtifactId；进度与 Artifact 分开重试；`AddArtifactAsync` 按 ArtifactId 幂等；永久元数据失败时写入可查询的 `PostCommitWarning`（多段 warning 合并；commit/completed 进度失败不阻断 Artifact）。
